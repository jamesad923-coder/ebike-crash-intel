...
#map {{ height:100%; }}
/* Rank badge pinned in each cell's top-left corner. pointer-events:none so
   clicks/hovers pass through to the map cell underneath. */
.cell-rank {{ font:700 11px -apple-system,"Segoe UI",Roboto,sans-serif;
  color:#fff; background:rgba(14,17,22,.72); border:1px solid rgba(255,255,255,.25);
  border-radius:5px; padding:0 4px; pointer-events:none; white-space:nowrap;
  text-shadow:0 1px 2px #000; }}
.maplibregl-popup-content {{ background:#161b22; color:#e6edf3; border:1px solid #2a313c;
  border-radius:8px; font-size:12px; padding:10px 12px; }}
.maplibregl-popup-content b {{ color:#f78166; }}
.maplibregl-popup-anchor-top .maplibregl-popup-tip {{ border-bottom-color:#161b22; }}
.maplibregl-popup-anchor-bottom .maplibregl-popup-tip {{ border-top-color:#161b22; }}
.maplibregl-popup-anchor-left .maplibregl-popup-tip {{ border-right-color:#161b22; }}
.maplibregl-popup-anchor-right .maplibregl-popup-tip {{ border-left-color:#161b22; }}
